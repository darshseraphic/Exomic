...
class SavingGoal {
  ...
  factory SavingGoal.fromMap(Map<dynamic, dynamic> map) => SavingGoal(
        ...
        dailyPaceRequired:
            (map['dailyPaceRequired'] as num?)?.toDouble() ?? 0.0,
        history: List<String>.from(map['history'] ?? []),
      );
}

final savingsProvider = StateProvider<List<SavingGoal>>((ref) {
  return ExomicDatabaseEngine.savingsBox.values
      .map((model) => SavingGoal(
            id: model.id,
            title: model.title,
            target: model.target,
...
class _SavingGoalsScreenState extends ConsumerState<SavingGoalsScreen> {
  ...
  void _calculateAndAddGoal(List<SavingGoal> currentGoals) async {
    ...
    if (title.isNotEmpty && target != null && _selectedDate != null) {
      ...
      final newGoal = SavingGoal(
        ...
        history: [
          "INIT: TARGET $target SET FOR ${_selectedDate!.year}-${_selectedDate!.month.toString().padLeft(2, '0')}-${_selectedDate!.day.toString().padLeft(2, '0')}"
        ],
      );
  ...
  void _processDeposit(
      SavingGoal goal, String amountText, String currencySymbol) async {
    ...
    if (remainingAmount > 0) {
      newDailyPace =
          daysLeft > 0 ? remainingAmount / daysLeft : remainingAmount;
    }
    ...
    final logEntry =
        "[$timeString] +$currencySymbol${depositAmount.toStringAsFixed(0)}";
    ...
    await ExomicDatabaseEngine.saveHistory(
        updatedLedger.map((e) => e.toMap()).toList());
                        ...
                        children: [
                          ...
                          InkWell(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                border: Border(
                                    bottom: BorderSide(color: specBorderColor)),
                              ),
                              child: Row(
                                mainAxisAlignment:
              ...
              child: goals.isEmpty
                  ? Center(
                      child: Text(
                        'NO ACTIVE POOLS',
                        style: TextStyle(
                            color: textSub, fontSize: 12, fontFamily: 'Inter'),
                      ),
                    )
                  : ListView.builder(
                            ...
                            children: [
                                      ...
                                      children: [
                                        Expanded(
                                          child: GestureDetector(
                                            onLongPress: () async {
                                              final updatedList = goals
                                                  .where((element) =>
                                                      element.id != goal.id)
                                                  .toList();
                                              ref
                                                  .read(
                                                      savingsProvider.notifier)
                                                  .state = updatedList;
                                              await ExomicDatabaseEngine
                                                  .savingsBox
                                                  .delete(goal.id);
                                            },
                                            child: Text(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              Text(
                                                isComplete
                                                    ? '--'
                                                    : '$currency${goal.dailyPaceRequired.toStringAsFixed(2)}',
                                                style: TextStyle(
                                                    color: textMain,
                                                    fontSize: 14,
                                                    fontFamily: 'Inter'),
                              ...
                              Container(
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    InkWell(
                                      onTap: () => _processDeposit(
                                          goal,
                                          _getDepositController(goal.id).text,
                                          currency),
                                      child: Container(
                                        height: 36,
                                        padding: const EdgeInsets.symmetric(
                              ...
                              if (goal.history.isNotEmpty) ...[
                                InkWell(
                                  onTap: () {
                                    final currentGoals =
                                        ref.read(savingsProvider);
                                    final goalIndex = currentGoals
                                        .indexWhere((g) => g.id == goal.id);
                                    if (goalIndex != -1) {
                                      final updatedList = [...currentGoals];
                                      updatedList[goalIndex] = goal.copyWith(
                                          isHistoryExpanded:
                                              !goal.isHistoryExpanded);
                                ...
                                AnimatedCrossFade(
                                  duration: const Duration(milliseconds: 250),
                                  sizeCurve: Curves.easeInOutCubic,
                                  crossFadeState: goal.isHistoryExpanded
                                      ? CrossFadeState.showSecond
                                      : CrossFadeState.showFirst,
                                  firstChild:
                                  ...
                                  secondChild: Container(
                                    ...
                                    child: ListView.builder(
                                      ...
                                      itemBuilder: (context, hIndex) {
                                        final logText = goal.history.reversed
                                            .toList()[hIndex];
                                        ...
                                        if (logText.startsWith('INIT:')) {
                                          ...
                                          final parts =
                                              logText.split(' SET FOR ');
                                          if (parts.length > 1) {
                                            subtitle = 'DEADLINE: ${parts[1]}';
                                          } else {
                                            subtitle = 'TARGET CONFIGURATION';
                                          }
                                          final targetStr = logText
                                              .replaceAll('INIT: TARGET ', '')
                                              .split(' SET FOR ')[0];
                                          ...
                                          final closeIndex =
                                              logText.indexOf(']');
                                          if (closeIndex != -1) {
                                            displayTime = logText.substring(
                                                1, closeIndex);
                                            displayAmount = logText
                                                .substring(closeIndex + 1)
                                                .trim();
